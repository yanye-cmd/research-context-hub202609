...
本索引按主题定位长期上下文。新增内容时优先更新已有主题文件；只有真正独立、以后需要单独检索的主题才新增文件。
...
| [`yanye-cmd/ccf-b-d2c-starlink202609`](https://github.com/yanye-cmd/ccf-b-d2c-starlink202609) | Reality-driven D2C / Starlink 主线 | 该主线正式状态唯一权威来源 |

当前科研状态不要从本 Hub 推断；去对应正式仓库读 `AGENTS.md`、`HANDOFF.md`、`tasks/NEXT_CODEX_TASK.md`。

## Conversations

跨对话压缩整理内容见 `conversations/`。优先保存长期 rationale，而不是机械聊天转录。
...
- 临时 idea：`research-sidequests/ideas/`
  ...
  - [`2026-09-14-gateway-multipath-d2c-context.md`](research-sidequests/ideas/2026-09-14-gateway-multipath-d2c-context.md)：Gateway/feeder 动态多卫星容量池、MPTCP/MPQUIC、INFOCOM 2026 PMPS、D2C 终端约束及 Rel-20 TN→NTN connected-mode mobility × SCS 频谱共存的持续讨论；仅为 continuation context。
  - [`2026-09-14-starlink-demand-allocation-control-loop-context.md`](research-sidequests/ideas/2026-09-14-starlink-demand-allocation-control-loop-context.md)：Starlink demand-driven allocation × AQM × transport 的长期讨论上下文；记录从专用冲突故事到 slow demand-driven allocation × transport feedback 母题、公开数据/emulator 路线与泛化边界；不代表正式科研状态。
...
仅保存非敏感、确有长期价值的沟通背景/经验；本仓库是 public。
